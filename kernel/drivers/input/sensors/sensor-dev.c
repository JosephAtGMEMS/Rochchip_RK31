...
#if defined (CONFIG_GS_GMA302)
static int gma_sysfs_update_active_status(struct sensor_private_data *sensor , int enable){
	int result;
	if(enable){
		mutex_lock(&sensor->operation_mutex);	
		//if(++sensor->start_count == 1)
		//{
			dev_info(&sensor->client->dev, "status_cur=%d : %d\n", sensor->status_cur, __LINE__);
			if(sensor->status_cur == SENSOR_OFF)
			{
				atomic_set(&(sensor->data_ready), 0);
				if ( (result = sensor->ops->active(sensor->client, 1, 0) ) < 0 ) 
				{
		        	mutex_unlock(&sensor->operation_mutex);
					dev_dbg(&sensor->client->dev, "%s:fail to active sensor,ret=%d\n",__func__,result);        
					goto error;           
		    	}
				if(sensor->pdata->irq_enable)
					enable_irq(sensor->client->irq);	//enable irq
				else
				{
					//PREPARE_DELAYED_WORK(&sensor->delaywork, sensor_delaywork_func);
					schedule_delayed_work(&sensor->delaywork, msecs_to_jiffies(sensor->pdata->poll_delay_ms));
					dev_dbg(&sensor->client->dev, "%s:use polling,delay=%d ms\n",__func__,sensor->pdata->poll_delay_ms);
				}
				sensor->status_cur = SENSOR_ON;
			}
		//}
	        mutex_unlock(&sensor->operation_mutex);
	        dev_dbg(&sensor->client->dev, "%s:GSENSOR enable\n", __func__);
	}
	else
		cancel_delayed_work_sync(&sensor->delaywork);
	sensor->status_cur = enable;
	return 0;
error:
	return result;
}
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_APP_SET_RATE:		
		...
		sensor->status_cur = SENSOR_ON;
	        ...
	        DBG("%s:GSENSOR_IOCTL_APP_SET_RATE OK\n", __func__);
		break;
	case GSENSOR_IOCTL_CALIBRATION:
		printk("%s:GSENSOR_IOCTL_CALIBRATION start\n", __func__);
#if defined (CONFIG_GS_GMA302)
	if(copy_from_user(&intBuf, (int*)argp, sizeof(intBuf))) return -EFAULT;
#endif
		mutex_lock(&sensor->operation_mutex);
		#if defined (CONFIG_GS_GMA302)
		result = sensor->ops->calibration(client, intBuf[0]);
		#else
		result = sensor->ops->calibration(client);
		#endif
		printk("result = %d\n", result);
		mutex_unlock(&sensor->operation_mutex);	
	    printk("%s:GSENSOR_IOCTL_CALIBRATION OK\n", __func__);
#if defined (CONFIG_GS_GMA302)
		//GMA_WriteCalibration(sensor, GMA_Offset_TXT);
		/* return the offset */
		for(i = 0; i < SENSOR_DATA_SIZE; ++i){
			intBuf[i] = sensor->offset.v[i];
			//dev_dbg(&client->dev, "intBuf[%d] = %d\n", i, intBuf[i]);
		}

		result = copy_to_user((int *)argp, &intBuf, sizeof(intBuf));
#endif
		break;
	case GSENSOR_IOCTL_GETDATA:
		mutex_lock(&sensor->data_mutex);
...
static ssize_t gma_enable_show(struct device *dev, struct device_attribute *attr, char *buf){
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	char str[2][16]={"ACC enable OFF","ACC enable ON"};
	int flag = atomic_read(&sensor->enable);
	return sprintf(buf, "%s\n", str[flag]);
}

static ssize_t gma_enable_store( struct device *dev, struct device_attribute *attr, char const *buf, size_t count){
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	int en = 0;
	if (NULL == buf)
		return -EINVAL;
	...
	en = en ? 1 : 0;
	dev_dbg(&sensor->client->dev, "buf=%x %x , en=%d : %d\n", buf[0], buf[1],en , __LINE__);
	atomic_set(&sensor->enable,en);
	gma_sysfs_update_active_status(sensor , en);
					...
					size_t count)
{
	...
	//atomic_set(&sensor->delay, (unsigned int) val);
	sensor->pdata->poll_delay_ms = (unsigned int) val;
	dev_dbg(&sensor->client->dev, "Driver attribute set delay =%lld\n", val);
	
	return count;
}
/* sysfs id show */
static ssize_t gma_chipinfo_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
				      ...
				      size_t count)
{
    ...
    if (!((side >= 1) && (side <= 9)))
		return -1;
	dev_info(&sensor->client->dev, "side= %ld\n", side);
	mutex_lock(&sensor->operation_mutex);
	ret = sensor->ops->calibration(sensor->client, side);
	mutex_unlock(&sensor->operation_mutex);	
	if(!ret)
		dev_info(&sensor->client->dev, "attribute calibration OK\n");
	/* save file */
	//GMA_WriteCalibration(sensor, GMA_Offset_TXT);
	return count;
}
/* sysfs offset show & store */
static ssize_t gma_offset_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	return sprintf(buf, "%d %d %d\n", sensor->offset.u.x, sensor->offset.u.y, sensor->offset.u.z);
}
...
{
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	sscanf(buf, "%d %d %d", (int *)&sensor->offset.v[0], (int *)&sensor->offset.v[1], (int *)&sensor->offset.v[2]);
	//GMA_WriteCalibration(sensor, GMA_Offset_TXT);
	return count;
}
/* sysfs data show */
...
{
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	return sprintf(buf, "%d %d %d\n", sensor->ewma_filter[0], sensor->ewma_filter[1], sensor->ewma_filter[2]);
}
				      ...
				      size_t count)
{
	...
	sscanf(buf, "%d %d %d", (int *)&sensor->ewma_filter[0], (int *)&sensor->ewma_filter[1], (int *)&sensor->ewma_filter[2]);
	if ((sensor->ewma_filter[0] == 2) || (sensor->ewma_filter[0] == 4) || (sensor->ewma_filter[0] == 8) || (sensor->ewma_filter[0] == 16))
	{
		ewma_init(&sensor->average[0], EWMA_FACTOR, sensor->ewma_filter[0]);
		dev_info(&sensor->client->dev, "ewma_init: sensor->ewma_filter[0]=%d \n", sensor->ewma_filter[0]);
	}
	if ((sensor->ewma_filter[1] == 2) || (sensor->ewma_filter[1] == 4) || (sensor->ewma_filter[1] == 8) || (sensor->ewma_filter[1] == 16))
	{
		ewma_init(&sensor->average[1], EWMA_FACTOR, sensor->ewma_filter[1]);
		dev_info(&sensor->client->dev, "ewma_init: sensor->ewma_filter[1]=%d \n", sensor->ewma_filter[1]);
	}
	if ((sensor->ewma_filter[2] == 2) || (sensor->ewma_filter[2] == 4) || (sensor->ewma_filter[2] == 8) || (sensor->ewma_filter[2] == 16))
	{
		ewma_init(&sensor->average[2], EWMA_FACTOR, sensor->ewma_filter[2]);
		dev_info(&sensor->client->dev, "ewma_init: sensor->ewma_filter[2]=%d \n", sensor->ewma_filter[2]);
	}
	return count;
}
						...
						char *buf)
{
	...
	i2c[0] = (unsigned char)atomic_read(&sensor->addr);
	err = sensor_rx_data(sensor->client, i2c, 6);
	if (err < 0)
		return err;
						...
						size_t count)
{
	...
	if (false == get_value_as_int(buf, count, &addr))
		return -EINVAL;
	dev_info(&sensor->client->dev, "addr=%d  \n", addr);
	if (addr < 0 || 128 < addr)
		return -EINVAL;

	atomic_set(&sensor->addr, addr);
						...
						size_t count)
{
	...
	if (false == get_value_as_int(buf, count, &value))
		return -EINVAL;
	//dev_info(&sensor->client->dev, "value=%d  \n", value);
	...
	i2c[0] = (unsigned char)atomic_read(&sensor->addr);
	i2c[1] = value;
	sensor_tx_data(sensor->client, i2c, 2);
	//dev_dbg(&sensor->client->dev, "i2c[0]=%d,i2c[1]=%d\n",i2c[0],i2c[1]);
	return count;
}
...
static DEVICE_ATTR(orientation,
		   0660,
		   NULL,
		   gsensor_set_orientation_online
		   );
...
static ssize_t wifi_chip_read(struct class *cls, struct class_attribute *attr, char *_buf)
{
	struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
	int count = 0;
	count = sprintf(_buf,"%s",sensor->ops->name);
	//printk("enter gsensor chip read!!!\n\n");
	return count;
}
...
int sensor_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	{
		/* Setup sysfs */
		if (sysfs_create_group(&sensor->input_dev->dev.kobj, &gma_acc_attribute_group) < 0){
	...
	if(sensor->ops->type == SENSOR_TYPE_ACCEL)	//gsensor need creat chip file for cali apk auto select
	{
		//printk("Enter gsensor creat  chip  file \n");
		result = class_create_file(sensor->input_dev->dev.class, &class_attr_chip);
		if(result)
			printk("creat read chip file fail!\n");
	}
